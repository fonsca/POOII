...
function render() {
  ...
  // aluno: redireciona sempre para a área dele, bloqueando rotas de admin
  if (user.role === "student") {
    if (!hash.startsWith("#/aluno")) return navigate("#/aluno");
      ...
      <div class="card login-card">
        ...
        <form id="auth-form">
          ...
          <p class="muted" style="margin-top:1rem;font-size:.85rem;text-align:center">
            Admin padrão: <b>admin@primeirachamada.com</b> / <b>123456</b>
          </p>
        </form>
      </div>
    </div>`;
...
async function renderStudentHome(user) {
  ...
  try {
    ...
    if (!data.student) {
      ...
      plannerEl.innerHTML = `<div class="empty">Nenhum planner disponível ainda. Aguarde seu mentor configurar seu plano.</div>`;
      return;
    }
    ...
    `).join("") : `<div class="empty">Nenhuma tarefa no seu planner ainda. Seu mentor irá adicionar em breve!</div>`;
